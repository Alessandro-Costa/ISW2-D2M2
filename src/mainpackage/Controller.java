...
public class Controller {
	...
	private List<MetricheModello> modelMetrics;
	private String outputFolder = "output/deliverable_2/";

	public void start(String projName) throws Exception {
		...
		catch(Exception e) { FileLogger.getLogger().warning("Errore nel caricamento ed esportazione del file");}
		for(Walk walk : walks) {
			evaluateWalk(walk);
		}
...
private Instances loadFile() {
	    ...
	    try { 
	    	loader.setSource(new File("/home/alessandro/eclipse-workspace/ISW2-D2M2/output/" /*+ projName*/ + "AVROMetrics.csv")); 
	    ...
	    try {
	    	saver.setFile(new File("/home/alessandro/eclipse-workspace/ISW2-D2M2/output/" + "AVROMetrics.arff"));
	    	saver.writeBatch(); 
	    } 
...
private void evaluateWalk(Walk walk) throws Exception {
...
/*private void chooseFeatureSelection(AbstractClassifier classifier, 
		String classifierName, Walk walk) throws Exception {
	...
	for(int balancingIndex = 0; balancingIndex < balancingNames.length; balancingIndex++) {
		...
		switch(balancingIndex) {
			...
			case 1: //Oversampling
				...
				try { 
					trainSet = Filter.useFilter(trainSet, resample);
					}
				finally {}
		   }
		}
		...
		/*private void evaluate(AbstractClassifier classifier, FilteredClassifier filteredClassifier,
			...
			if(!Double.isNaN(evaluation.precision(classIndex)) && !Double.isNaN(evaluation.recall(classIndex)) && !Double.isNaN(evaluation.areaUnderROC(classIndex)) &&
				...
				double[] percents = {walk.getPercTrainingDataset(), walk.getPercDefectiveTrain(), walk.getPercDefectiveTest()};
				...
				modelMetrics.add(new MetricheModello(projName, attributes, walk.getTrainIndex(),percents , trueFalsePositiveNegative, metrics));
...
/*private void printEvaluation(Instances train, Instances test, Walk walk, Evaluation eval, String classifier, String balancing, String feature) {
	...
	private void generateCSV(List<MetricheModello> modelMetrics) 
	{
		...
		try (BufferedWriter br = new BufferedWriter(
			...
			for(MetricheModello modelMetric : modelMetrics) {
	...
	public static Controller getIstance() {
		if(istance == null) istance = new Controller();
		return istance;
	}
